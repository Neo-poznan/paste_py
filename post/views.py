...
from pastebin.settings import SERVER_URL
from post.s3 import upload_file_to_s3
...
async def get_post_view(request, post_key):
    ...
    if not is_viewed:
        ...
        post_db_row.views_count += 1
    # обновляем последнюю дату вызова
    await post_db_row.asave()
    # получаем текст поста
    post_content = await get_and_cache_post_content(post_key)
    return render(request, 'post/post.html', context={'post_text': post_content, 'views': post_db_row.views_count})
